...
export class ApprovalsPage implements OnDestroy {
  ...
  readonly concludedItems = computed(() =>
  ...
  signSelectedOrder(): void {
    ...
    this.pedidoService
      ...
      .subscribe({
        next: (response) => {
          const isDone = response.order.status === 'CONCLUIDO';
          const warning =
          ...
          this.toastMessage.set(
            isDone
              ? `Pedido ${order.orderNumber} concluido para faturamento.` + warning
              : `Etapa assinada. Pedido agora está em ${this.statusLabel(response.order.status)}.` + warning
          );
          if (isDone) {
            this.downloadOrderById(order.id, `${order.orderNumber}-concluido.pdf`);
          }
